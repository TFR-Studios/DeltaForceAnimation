...
| 核电站功率动画 | `animation_3/animation_data.json` + `animation_3/png resources/{99999,百叶窗}/` | — | 底层 HUD 动画上叠加两条 359 帧 PNG 序列(默认开启);两条序列可在侧栏「图片图层」里改色/调透明度 |
...
- **文字图层**:改内容/颜色/不透明度,「重置」按钮(SVG 图标)一键还原;
...
- **图标选择**(位置暴露):
  ...
  - 「上传自定义图标」(前置 SVG 加号图标):上传 PNG/JPG/WebP(≤2MB,会话内有效),应用到当前选项卡对应的段;
...
| 选项 | 格式 | 说明 |
|---|---|---|
| 未勾选透明 | MP4 / AVI | H.264 30 Mbps,分辨率跟随当前动画(撤离 1920×1080 / 位置暴露 3840×1080);AVI 为 H.264-in-AVI(带 avcC),PotPlayer/VLC/剪映/FFmpeg 系通用 |
...
- **轨道遮罩在 canvas 渲染器下不可靠**:遮罩源被多图层共用时合成会偏暗/丢内容(见「兼容导出」);
  预览请用 SVG 渲染器(默认),导出请开启「兼容导出」。这是 lottie-web canvas 渲染器自身的问题,站点侧只能绕过;
