...
// --- 3) 构造/更新序列图层(替换旧的 image_ccr_* asset) ---
j.assets = j.assets.filter((a) => !String(a.id).startsWith('image_ccr_'));
const seqLayer = {
  ddd: 0,
  ind: 56,
  ty: 2,
...
const existingSeq = j.layers.findIndex((l) => l.ty === 2 && String(l.nm) === 'ccreptile');
if (existingSeq >= 0) {
  j.layers[existingSeq] = seqLayer;
  console.log('更新序列图层 @数组位置 ' + existingSeq);
} else {
  j.layers.splice(removeIdx >= 0 ? removeIdx : 55, 0, seqLayer);
  console.log('插入序列图层 ind=' + seqLayer.ind + ' nm=' + seqLayer.nm);
}
